 ...
 * @author Ksu
 * @typedef {Object} dbTransactionsBlocksListBasic
 * @property {number} id
...
class DbTransactionsTodo {
    ...
    constructor(db, currencyCode, divider) {
        ...
        /** @private @member {string} **/
        this.tableName = `transactions_blocks_list_basics_${this.currencyCode.toLowerCase()}_${divider}`
    }
     ...
     */
    async releaseLocked() {
        let count = `SELECT id, _scanned_time FROM ${this.tableName} 
                          WHERE _scanned<0 AND _scanned_time < (NOW() - INTERVAL '20 MINUTE') 
                          LIMIT 1`
        let lock = await this.db.query(count, [])
        if (!lock) {
            return true
        }
        let sql = `UPDATE ${this.tableName} SET _scanned=0 
                          WHERE _scanned<0 AND _scanned_time < (NOW() - INTERVAL '20 MINUTE')`
    ...
    /**
     * @param {number} limit
     * @return {Promise<dbTransactionsBlocksListBasic[]|boolean>}
     */
    async getNextAndLock(limit) {
        let index = Math.floor(Math.random() * MAX_RAND) + 1

        let sql1 = `SELECT id FROM ${this.tableName} WHERE _scanned=0 AND _removed=0 ORDER BY block_number LIMIT $2`
        ...
        let sql2 = `SELECT id, inner_block_id AS "innerBlockID", block_number AS "blockNumber", transaction_hash AS "transactionHash"
                    FROM ${this.tableName} WHERE _removed=0 AND _scanned=$1 LIMIT $2`
        try {
            let lock1 = await this.db.query(
                `UPDATE ${this.tableName} SET _scanned = $1, _scanned_time=NOW() WHERE id IN (${sql1})`,
                params1
            )
    ...
    async finishLocked(data) {
        ...
        let sql = `UPDATE ${this.tableName} SET _scanned_time=NOW(), _scanned = 99999 WHERE id IN (`
...
module.exports.init = function(db, currencyCode, divider) {
    return new DbTransactionsTodo(db, currencyCode, divider)
}
